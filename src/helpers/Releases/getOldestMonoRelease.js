// sort mono release list to make the oldest mono release as the first index
const getOldestMonoRelease = (releaseList) => {
  const monoVersions = releaseList.filter((version) => version.includes('mono'));

  const oldestMonoVersion = [...monoVersions].sort((a, b) => {
  ...
  const defaultRelease = oldestMonoVersion[0];
...
module.exports = getOldestMonoRelease;
